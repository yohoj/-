...
void init_destination (j_compress_ptr cinfo)
{
    ...
    dest->buffer = (JOCTET *)malloc(DEFAULT_JPEG_SIZE * sizeof(JOCTET));
...
boolean empty_output_buffer (j_compress_ptr cinfo)
{
    ...
    tmp = (u_char *)realloc(dest->buffer, (dest->total_buffer_size + DEFAULT_JPEG_SIZE) * sizeof(JOCTET));
	if(tmp == NULL)
	{
...
//METHODDEF(long)
long get_jpeg_size (j_compress_ptr cinfo)
{
	my_dest_ptr dest = (my_dest_ptr) cinfo->dest;
...
//METHODDEF(void)
void jpeg_buff_free (j_compress_ptr cinfo)
{
	my_dest_ptr dest = (my_dest_ptr) cinfo->dest;
